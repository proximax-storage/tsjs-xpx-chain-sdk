...
const SignSchema_1 = require("../../../src/core/crypto/SignSchema");
const format_1 = require("../../../src/core/format");
const Convert_1 = require("../../../src/core/format/Convert");
describe('key pair', () => {
    const randomKeyPair = () => crypto_1.KeyPair.createKeyPairFromPrivateKeyString(format_1.Convert.uint8ToHex(crypto_1.Crypto.randomBytes(32)));
    ...
    describe('construction', () => {
        it('can extract from private key test vectors', () => {
            ...
            // Sanity:
            chai_1.expect(Private_Keys.length).equal(Expected_Public_Keys.length);
            for (let i = 0; i < Private_Keys.length; ++i) {
                // Arrange:
                const privateKeyHex = Private_Keys[i];
                const expectedPublicKey = Expected_Public_Keys[i];
                // Act:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(privateKeyHex);
                // Assert:
                const message = ` from ${privateKeyHex}`;
                chai_1.expect(format_1.Convert.uint8ToHex(keyPair.publicKey), `public ${message}`).equal(expectedPublicKey);
                chai_1.expect(format_1.Convert.uint8ToHex(keyPair.privateKey), `private ${message}`).equal(privateKeyHex);
            }
        });
        it('cannot extract from invalid private key', () => {
            ...
            const invalidPrivateKeys = [
                '',
                '53C659B47C176A70EB228DE5C0A0FF391282C96640C2A42CD5BBD0982176AB',
                '53C659B47C176A70EB228DE5C0A0FF391282C96640C2A42CD5BBD0982176AB1BBB',
            ];
            // Act:
            invalidPrivateKeys.forEach((privateKey) => {
                // Assert:
                chai_1.expect(() => {
                    crypto_1.KeyPair.createKeyPairFromPrivateKeyString(privateKey);
                }, `from ${privateKey}`)
    ...
    describe('NIS1 test vector - Keccak', () => {
        it('can extract from private key test vectors', () => {
            ...
            // Sanity:
            chai_1.expect(Nis1_Private_Key.length).equal(Expected_Public_Keys.length);
            for (let i = 0; i < Nis1_Private_Key.length; ++i) {
                // Arrange:
                const privateKeyHex = Nis1_Private_Key[i];
                const expectedPublicKey = Expected_Public_Keys[i];
                // Act:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(privateKeyHex, SignSchema_1.SignSchema.KECCAK_REVERSED_KEY);
                // Assert:
                const message = ` from ${privateKeyHex}`;
                chai_1.expect(format_1.Convert.uint8ToHex(keyPair.publicKey).toUpperCase(), `public ${message}`).equal(expectedPublicKey.toUpperCase());
                chai_1.expect(format_1.Convert.uint8ToHex(keyPair.privateKey).toUpperCase(), `private ${message}`).equal(privateKeyHex.toUpperCase());
            }
        });
        it('cannot extract from invalid private key', () => {
            // Arrange:
    ...
    describe('Catapult test vector - SHA3', () => {
        it('can extract from private key test vectors', () => {
            ...
            // Sanity:
            chai_1.expect(Private_Key.length).equal(Expected_Public_Keys.length);
    ...
    describe('sign & verify- Test Vector', () => {
        ...
        it('NIS1', () => {
            ...
            for (let i = 0; i < Nis1_Private_Key.length; ++i) {
                // Arrange:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Nis1_Private_Key[i], SignSchema_1.SignSchema.KECCAK_REVERSED_KEY);
                const payload = Convert_1.Convert.hexToUint8(Nis1_Data[i]);
                // Act:
                const signature = crypto_1.KeyPair.sign(keyPair, payload, SignSchema_1.SignSchema.KECCAK_REVERSED_KEY);
                // Assert:
                const message = ` from ${Nis1_Private_Key[i]}`;
                chai_1.expect(Convert_1.Convert.uint8ToHex(crypto_1.KeyPair.sign(keyPair, payload, SignSchema_1.SignSchema.KECCAK_REVERSED_KEY)).toUpperCase(), `private ${message}`).to.deep.equal(Expected_Signature[i].toUpperCase());
                const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, signature, SignSchema_1.SignSchema.KECCAK_REVERSED_KEY);
                chai_1.expect(isVerified, `private ${message}`).to.equal(true);
            }
        });
        /**
        ...
        it('CATAPULT', () => {
            ...
            for (let i = 0; i < Catapult_Private_Key.length; ++i) {
                // Arrange:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Catapult_Private_Key[i], SignSchema_1.SignSchema.SHA3);
                const payload = Convert_1.Convert.hexToUint8(Catapult_Data[i]);
                // Act:
                const signature = crypto_1.KeyPair.sign(keyPair, payload, SignSchema_1.SignSchema.SHA3);
                // Assert:
                const message = ` from ${Catapult_Private_Key[i]}`;
                chai_1.expect(Convert_1.Convert.uint8ToHex(signature).toUpperCase(), `private ${message}`).to.deep.equal(Expected_Signature[i].toUpperCase());
                const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, signature);
                chai_1.expect(isVerified, `private ${message}`).to.equal(true);
            }
        });
    });
    describe('sign', () => {
        it('fills the signature', () => {
            ...
            const payload = crypto_1.Crypto.randomBytes(100);
            // Act:
            const signature = crypto_1.KeyPair.sign(keyPair, payload);
            // Assert:
            chai_1.expect(signature).to.not.deep.equal(new Uint8Array(Signature_Size));
        });
        it('returns same signature for same data signed by same key pairs', () => {
            // Arrange:
            const privateKey = format_1.Convert.uint8ToHex(crypto_1.Crypto.randomBytes(Private_Key_Size));
            const keyPair1 = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(privateKey);
            const keyPair2 = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(privateKey);
            const payload = crypto_1.Crypto.randomBytes(100);
            // Act:
            const signature1 = crypto_1.KeyPair.sign(keyPair1, payload);
            const signature2 = crypto_1.KeyPair.sign(keyPair2, payload);
            // Assert:
            chai_1.expect(signature2).to.deep.equal(signature1);
        });
        it('returns different signature for same data signed by different key pairs', () => {
            ...
            const signature2 = crypto_1.KeyPair.sign(keyPair2, payload);
            // Assert:
            chai_1.expect(signature2).to.not.deep.equal(signature1);
        });
        it('cannot sign unsupported data type', () => {
            // Arrange:
            const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Private_Keys[0]);
            // Assert:
            chai_1.expect(() => {
                crypto_1.KeyPair.sign(keyPair, {});
    ...
    describe('verify', () => {
        it('returns true for data signed with same key pair', () => {
            ...
            const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, signature);
            // Assert:
            chai_1.expect(isVerified).to.equal(true);
        });
        it('returns false for data signed with different key pair', () => {
            ...
            const keyPair2 = randomKeyPair();
            const payload = crypto_1.Crypto.randomBytes(100);
            const signature = crypto_1.KeyPair.sign(keyPair1, payload);
            // Act:
            const isVerified = crypto_1.KeyPair.verify(keyPair2.publicKey, payload, signature);
            // Assert:
            chai_1.expect(isVerified).to.equal(false);
        });
        it('returns false if signature has been modified', () => {
            ...
            for (let i = 0; i < Signature_Size; i += 4) {
                ...
                const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, signature);
                // Assert:
                chai_1.expect(isVerified, `signature modified at ${i}`).to.equal(false);
            }
        });
        it('returns false if payload has been modified', () => {
            // Arrange:
            const keyPair = randomKeyPair();
            const payload = crypto_1.Crypto.randomBytes(44);
            for (let i = 0; i < payload.length; i += 4) {
                ...
                const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, signature);
                // Assert:
                chai_1.expect(isVerified, `payload modified at ${i}`).to.equal(false);
            }
        });
        ...
        it('cannot verify non canonical signature', () => {
            ...
            const canonicalSignature = crypto_1.KeyPair.sign(keyPair, payload);
            ...
            // Act:
            const isCanonicalVerified = crypto_1.KeyPair.verify(keyPair.publicKey, payload, canonicalSignature);
            const isNonCanonicalVerified = crypto_1.KeyPair.verify(keyPair.privateKey, payload, nonCanonicalSignature);
            // Assert:
            chai_1.expect(isCanonicalVerified).to.equal(true);
            chai_1.expect(isNonCanonicalVerified).to.equal(false);
        });
    });
    describe('test vectors', () => {
        ...
        function assertCanSignTestVectors(dataTransform) {
            // Sanity:
            chai_1.expect(Private_Keys.length).equal(Input_Data.length);
            chai_1.expect(Private_Keys.length).equal(Expected_Signatures.length);
            for (let i = 0; i < Private_Keys.length; ++i) {
                // Arrange:
                const inputData = dataTransform(Input_Data[i]);
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Private_Keys[i]);
                // Act:
                const signature = crypto_1.KeyPair.sign(keyPair, inputData);
                // Assert:
                const message = `signing with ${Private_Keys[i]}`;
                chai_1.expect(format_1.Convert.uint8ToHex(signature), message).equal(Expected_Signatures[i]);
            }
        }
        ...
        it('can sign test vectors as binary', () => {
            // Assert:
            assertCanSignTestVectors((data) => format_1.Convert.hexToUint8(data));
        });
        function assertCanVerifyTestVectors(dataTransform) {
            ...
            for (let i = 0; i < Private_Keys.length; ++i) {
                ...
                const signature = crypto_1.KeyPair.sign(keyPair, inputData);
                // Act:
                const isVerified = crypto_1.KeyPair.verify(keyPair.publicKey, inputData, signature);
                // Assert:
                const message = `verifying with ${Private_Keys[i]}`;
                chai_1.expect(isVerified, message).equal(true);
            }
        }
        ...
        it('can verify test vectors as binary', () => {
            // Assert:
            assertCanVerifyTestVectors((data) => format_1.Convert.hexToUint8(data));
        });
    });
    describe('derive shared key', () => {
        ...
        it('fails if salt is wrong size', () => {
            ...
            const keyPair = randomKeyPair();
            const publicKey = crypto_1.Crypto.randomBytes(32);
            const salt = crypto_1.Crypto.randomBytes(Salt_Size + 1);
            // Act:
            chai_1.expect(() => {
                crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey, salt);
            })
        ...
        it('derives same shared key for both partners', () => {
            ...
            const keyPair1 = randomKeyPair();
            const keyPair2 = randomKeyPair();
            const salt = crypto_1.Crypto.randomBytes(Salt_Size);
            // Act:
            const sharedKey1 = crypto_1.KeyPair.deriveSharedKey(keyPair1, keyPair2.publicKey, salt);
            const sharedKey2 = crypto_1.KeyPair.deriveSharedKey(keyPair2, keyPair1.publicKey, salt);
            // Assert:
            chai_1.expect(sharedKey1).to.deep.equal(sharedKey2);
        });
        it('derives different shared keys for different partners', () => {
            // Arrange:
            const keyPair = randomKeyPair();
            const publicKey1 = crypto_1.Crypto.randomBytes(32);
            const publicKey2 = crypto_1.Crypto.randomBytes(32);
            const salt = crypto_1.Crypto.randomBytes(Salt_Size);
            // Act:
            const sharedKey1 = crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey1, salt);
            const sharedKey2 = crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey2, salt);
            // Assert:
            chai_1.expect(sharedKey1).to.not.deep.equal(sharedKey2);
        });
        it('can derive deterministic shared key from well known inputs', () => {
            // Arrange:
            const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString('8F545C2816788AB41D352F236D80DBBCBC34705B5F902EFF1F1D88327C7C1300');
            const publicKey = format_1.Convert.hexToUint8('BF684FB1A85A8C8091EE0442EDDB22E51683802AFA0C0E7C6FE3F3E3E87A8D72');
            const salt = format_1.Convert.hexToUint8('422C39DF16AAE42A74A5597D6EE2D59CFB4EEB6B3F26D98425B9163A03DAA3B5');
            // Act:
            const sharedKey = crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey, salt);
            // Assert:
            chai_1.expect(format_1.Convert.uint8ToHex(sharedKey)).to.equal('007FD607264C64C7BB83509E7CFA96E0FEAF34A373CDA75FACAA4DE9E141257B');
        });
    });
    /**
    ...
    describe('derive shared key - Test Vecto NIS1', () => {
        it('derive shared key using keccak', () => {
            ...
            for (let i = 0; i < Nis1_Private_Key.length; ++i) {
                // Arrange:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Nis1_Private_Key[i], SignSchema_1.SignSchema.KECCAK_REVERSED_KEY);
                const publicKey = Convert_1.Convert.hexToUint8(Nis1_Public_Keys[i]);
                const salt = Convert_1.Convert.hexToUint8(Nis1_Salt[i]);
                // Act:
                const sharedKey = Convert_1.Convert.uint8ToHex(crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey, salt, SignSchema_1.SignSchema.KECCAK_REVERSED_KEY));
                // Assert:
                const message = ` from ${Nis1_Private_Key[i]}`;
                chai_1.expect(sharedKey.toUpperCase()).to.deep.equal(Expected_Derived_Key[i].toUpperCase());
            }
        });
    });
    /**
    ...
    describe('derive shared key - Test Vecto Catapult', () => {
        it('derive shared key using sha3', () => {
            ...
            for (let i = 0; i < Private_Key.length; ++i) {
                // Arrange:
                const keyPair = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(Private_Key[i]);
                const publicKey = Convert_1.Convert.hexToUint8(Public_Keys[i]);
                const salt = Convert_1.Convert.hexToUint8(Salt[i]);
                // Act:
                const sharedKey = Convert_1.Convert.uint8ToHex(crypto_1.KeyPair.deriveSharedKey(keyPair, publicKey, salt));
                // Assert:
