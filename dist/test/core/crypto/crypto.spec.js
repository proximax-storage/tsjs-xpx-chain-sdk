...
describe('crypto tests', () => {
    it('Can derive a key from password and count', () => {
        ...
        // Act:
        const result = crypto_1.Crypto.derivePassSha(password, count);
        // Assert:
        chai_1.expect(result.priv).equal(expectedKey);
    });
    it('Can encrypt a private key', () => {
        ...
        const expectedKey = '8cd87bc513857a7079d182a6e19b370e907107d97bd3f81a85bcebcc4b5bd3b5';
        // Act:
        const result = crypto_1.Crypto.encodePrivateKey(privateKey, password);
        const pass = crypto_1.Crypto.derivePassSha(password, 20);
        // Assert:
        chai_1.expect(pass.priv).equal(expectedKey);
        chai_1.expect(result.iv.length).equal(16 * 2);
        chai_1.expect(result.ciphertext.length).equal(48 * 2);
    });
    it('Can decrypt a private key', () => {
        ...
        const obj = {
            ...
            iv: format_1.Convert.hexToUint8(iv),
            key: format_1.Convert.hexToUint8(key),
        };
        // Act:
        const decrypted = crypto_1.Crypto.decrypt(obj);
        // Assert:
        chai_1.expect(decrypted).equal(expectedPrivateKey);
    });
    it('Can encrypt and decrypt private Key', () => {
        ...
        const obj = {
            ...
            iv: format_1.Convert.hexToUint8(result.iv),
            key: format_1.Convert.hexToUint8(pass.priv),
        };
        const decrypted = crypto_1.Crypto.decrypt(obj);
        // Assert:
        chai_1.expect(privateKey).equal(decrypted);
    });
    describe('Encrypt private key edge-cases', () => {
        it('Encryption throw error if no password', () => {
            ...
            const privateKey = '2a91e1d5c110a8d0105aad4683f962c2a56663a3cad46666b16d243174673d90';
            // Act:
            const result = crypto_1.Crypto.encodePrivateKey.bind(null, privateKey, password);
            // Assert:
            chai_1.expect(result).to.throw('Missing argument !');
        });
    ...
    it('Can decrypt private key of pass:enc wallets', () => {
        ...
        const mainAlgo = model_1.WalletAlgorithm.Pass_enc;
        const expectedPrivateKey = '2a91e1d5c110a8d0105aad4683f962c2a56663a3cad46666b16d243174673d90';
        // Act:
        const result = crypto_1.Crypto.passwordToPrivateKey(common, walletAccount, mainAlgo);
        // Assert:
        chai_1.expect(result).equal(true);
        chai_1.expect(common.privateKey).equal(expectedPrivateKey);
    });
    it('Can decrypt private key of pass:bip32 wallets', () => {
        ...
        const mainAlgo = model_1.WalletAlgorithm.Pass_bip32;
        const expectedPrivateKey = '2a91e1d5c110a8d0105aad4683f962c2a56663a3cad46666b16d243174673d90';
        // Act:
        const result = crypto_1.Crypto.passwordToPrivateKey(common, walletAccount, mainAlgo);
        // Assert:
    ...
    it('Can decrypt private key of pass:6k wallets', () => {
        ...
        const mainAlgo = model_1.WalletAlgorithm.Pass_6k;
    ...
    describe('Decrypt private key edge-cases', () => {
        it('Private key decryption throw error if no algo', () => {
            ...
            const mainAlgo = '';
            // Act:
            const result = crypto_1.Crypto.passwordToPrivateKey.bind(null, common, walletAccount, mainAlgo);
            // Assert:
            chai_1.expect(result).to.throw('Missing argument !');
        });
        ...
        it('Decryption of pass:6k wallets return false if encrypted data but no iv', () => {
            ...
            chai_1.expect(common.privateKey).equal('');
        });
    ...
    it('Can encode and decode message', () => {
        ...
        const encryptedMessage = crypto_1.Crypto.encode(senderPriv, recipientPublic, message);
        ...
        const decrypted = crypto_1.Crypto.decode(recipientPriv, senderPublic, encryptedMessage);
        chai_1.expect(decrypted).equal(expectedMessage);
    });
    it('Can encode a message and failed decode with wrong key', () => {
        ...
        const expectedMessage = 'NEM is awesome !';
        const decrypted = crypto_1.Crypto.decode(recipientPriv, senderPublic, encryptedMessage);
        chai_1.expect(decrypted).not.equal(expectedMessage);
    });
    describe('Encode & decode message edge-cases', () => {
        it('Message encoding throw error if no sender private key', () => {
            ...
            const message = 'NEM is awesome !';
            // Act:
            const result = crypto_1.Crypto.encode.bind(null, senderPriv, recipientPublic, message);
            // Assert:
            chai_1.expect(result).to.throw();
        });
        ...
        it('Message decoding throw error if no recipient private key', () => {
            ...
            // Act:
            const result = crypto_1.Crypto.decode.bind(null, recipientPriv, senderPublic, encryptedMessage);
            // Assert:
            chai_1.expect(result).to.throw();
        });
    ...
    it('Can encrypt and decrypt private key for mobile', () => {
        ...
        const password = 'TestTest';
        // Act:
        const result = crypto_1.Crypto.toMobileKey(password, privateKey);
        ...
        const obj = {
            ...
            iv: format_1.Convert.hexToUint8(iv),
            key: format_1.Convert.hexToUint8(key.toString()),
        };
        const decrypted = crypto_1.Crypto.decrypt(obj);
        // Assert:
        chai_1.expect(encrypted.length).equal(128);
        chai_1.expect(salt.toString().length).equal(32 * 2);
        chai_1.expect(decrypted).equal(privateKey);
    });
});
