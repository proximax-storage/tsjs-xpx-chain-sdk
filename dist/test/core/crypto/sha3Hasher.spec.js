...
const SHA3Hasher_1 = require("../../../src/core/crypto/SHA3Hasher");
    ...
    function addSha3Tests(length, expectedOutputs) {
        describe('func', () => {
            it('can hash test vectors', () => {
                // Sanity:
                chai_1.expect(expectedOutputs.length).equal(inputs.length);
                for (let i = 0; i < inputs.length; ++i) {
                    // Arrange:
                    const inputHex = inputs[i];
                    const inputBuffer = format_1.Convert.hexToUint8(inputHex);
                    const expectedHash = expectedOutputs[i];
                    // Act:
                    const hash = new Uint8Array(length);
                    SHA3Hasher_1.SHA3Hasher.func(hash, inputBuffer, length);
                    // Assert:
                    chai_1.expect(format_1.Convert.uint8ToHex(hash), `hashing ${inputHex}`).equal(expectedHash);
                }
            });
        });
        describe('object', () => {
            it('can hash test vectors', () => {
                ...
                for (let i = 0; i < inputs.length; ++i) {
                    ...
                    const inputBuffer = format_1.Convert.hexToUint8(inputHex);
                    const expectedHash = expectedOutputs[i];
                    const hasher = SHA3Hasher_1.SHA3Hasher.createHasher(length);
                    hasher.reset();
            ...
            it('cannot hash unsupported data type', () => {
                ...
                hasher.reset();
                // Act:
                chai_1.expect(() => hasher.update({})).to.throw('unsupported data type');
            });
...
//# sourceMappingURL=sha3Hasher.spec.js.map