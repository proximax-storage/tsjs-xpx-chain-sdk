...
describe('uint64', () => {
    describe('compact', () => {
        it('can compact 32 bit value', () => {
            // Act:
            const result = format_1.RawUInt64.compact([0x12345678, 0x00000000]);
            // Assert:
            chai_1.expect(result).to.equal(0x12345678);
        });
        it('can compact less than max safe integer', () => {
            // Act:
            const result = format_1.RawUInt64.compact([0x00ABCDEF, 0x000FDFFF]);
            // Assert:
            chai_1.expect(result).to.equal(0xFDFFF00ABCDEF);
        });
        it('can compact max safe integer', () => {
            // Sanity:
            chai_1.expect(0x1FFFFFFFFFFFFF).to.equal(Number.MAX_SAFE_INTEGER);
            // Act:
            const result = format_1.RawUInt64.compact([0xFFFFFFFF, 0x001FFFFF]);
            // Assert:
            chai_1.expect(result).to.equal(Number.MAX_SAFE_INTEGER);
        });
        it('cannot compact min unsafe integer', () => {
            // Sanity:
            chai_1.expect(0x0020000000000000 + 1).to.equal(0x0020000000000000);
            // Act:
            const result = format_1.RawUInt64.compact([0x00000000, 0x00200000]);
            // Assert:
            chai_1.expect(result).to.deep.equal([0x00000000, 0x00200000]);
        });
        it('cannot compact greater than min unsafe integer', () => {
            // Act:
            const result = format_1.RawUInt64.compact([0xF0000000, 0x01000D00]);
            // Assert:
            chai_1.expect(result).to.deep.equal([0xF0000000, 0x01000D00]);
        });
    });
    describe('fromUint', () => {
        ...
        successTestCases.forEach(testCase => {
            it(`can parse numeric with ${testCase.description} significant digits`, () => {
                // Act:
                const value = format_1.RawUInt64.fromUint(testCase.number);
                // Assert:
                chai_1.expect(value).to.deep.equal(testCase.uint64);
            });
        });
    });
    const hexTestCases = [{
    ...
    describe('fromBytes', () => {
        hexTestCases.forEach(testCase => {
            it(`can parse byte array with ${testCase.description} significant digits`, () => {
                ...
                const bytes = format_1.Convert.hexToUint8(testCase.str).reverse();
                // Act:
                const value = format_1.RawUInt64.fromBytes(bytes);
                // Assert:
        ...
        it('cannot parse byte array with invalid size into uint64', () => {
            ...
            [0, 3, 4, 5, 7, 9].forEach(size => {
                chai_1.expect(() => {
                    format_1.RawUInt64.fromBytes(new Uint8Array(size));
    ...
    describe('fromBytes32', () => {
        ...
        fromBytes32TestCases.forEach(testCase => {
            it(`can parse byte array with ${testCase.description} significant digits`, () => {
                ...
                const bytes = format_1.Convert.hexToUint8(testCase.str).reverse();
                // Act:
                const value = format_1.RawUInt64.fromBytes32(bytes);
                // Assert:
                chai_1.expect(value).to.deep.equal(testCase.value);
            });
        });
        it('cannot parse byte array with invalid size into uint64', () => {
            ...
            [0, 3, 5, 7, 8, 9].forEach(size => {
                chai_1.expect(() => {
                    format_1.RawUInt64.fromBytes32(new Uint8Array(size));
                }, `size ${size}`).to.throw(errorMessage);
            });
        });
    });
    describe('fromHex', () => {
        hexTestCases.forEach(testCase => {
            it(`can parse hex string with ${testCase.description} significant digits`, () => {
                // Act:
                const value = format_1.RawUInt64.fromHex(testCase.str);
                // Assert:
                chai_1.expect(value).to.deep.equal(testCase.value);
            });
        });
        it('cannot parse hex string with invalid characters into uint64', () => {
            // Assert:
            chai_1.expect(() => {
                format_1.RawUInt64.fromHex('0000000012345G78');
        ...
        it('cannot parse hex string with invalid size into uint64', () => {
            ...
            // Assert:
            chai_1.expect(() => {
                format_1.RawUInt64.fromHex('');
            }).to.throw(errorMessage); // empty string
            chai_1.expect(() => {
                format_1.RawUInt64.fromHex('1');
            }).to.throw(errorMessage); // odd number of chars
            chai_1.expect(() => {
                format_1.RawUInt64.fromHex('ABCDEF12');
            }).to.throw(errorMessage); // too short
            chai_1.expect(() => {
                format_1.RawUInt64.fromHex('1234567890ABCDEF12');
    ...
    describe('toHex', () => {
        hexTestCases.forEach(testCase => {
            it(`can format hex string with ${testCase.description} significant digits`, () => {
                // Act:
                const str = format_1.RawUInt64.toHex(testCase.value);
                // Assert:
                chai_1.expect(str).to.equal(testCase.str);
            });
        });
    });
    describe('isZero', () => {
        ...
        zeroTestCases.forEach(testCase => {
            it(`returns ${testCase.isZero} when ${testCase.description}`, () => {
                // Act:
                const isZero = format_1.RawUInt64.isZero(testCase.value);
                // Assert:
                chai_1.expect(isZero).to.equal(testCase.isZero);
            });
        });
    });
});
//# sourceMappingURL=RawUInt64.spec.js.map