...
describe('address', () => {
    describe('stringToAddress', () => {
        function assertCannotCreateAddress(encoded, message) {
            // Assert:
            chai_1.expect(() => {
                format_1.RawAddress.stringToAddress(encoded);
            }).to.throw(message);
        }
        it('can create address from valid encoded address', () => {
            ...
            // Act:
            const decoded = format_1.RawAddress.stringToAddress(encoded);
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded)).to.equal(true);
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal(expectedHex);
        });
        it('cannot create address from encoded string with wrong length', () => {
            // Assert:
            assertCannotCreateAddress('NC5J5DI2URIC4H3T3IMXQS25PWQWZIPEV6EV7LASABCDEFGH', 'NC5J5DI2URIC4H3T3IMXQS25PWQWZIPEV6EV7LASABCDEFGH does not represent a valid encoded address');
        });
    ...
    describe('addressToString', () => {
        it('can create encoded address from address', () => {
            ...
            // Act:
            const encoded = format_1.RawAddress.addressToString(format_1.Convert.hexToUint8(decodedHex));
            // Assert:
            chai_1.expect(encoded).to.equal(expected);
        });
    });
    describe('publicKeyToAddress', () => {
        it('can create address from public key for well known network', () => {
            ...
            const publicKey = format_1.Convert.hexToUint8('3485D98EFD7EB07ADAFCFD1A157D89DE2796A95E780813C0258AF3F5F84ED8CB');
            // Act:
            const decoded = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Mijin_Identifier);
            // Assert:
            chai_1.expect(decoded[0]).to.equal(Network_Mijin_Identifier);
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded)).to.equal(true);
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal(expectedHex);
        });
        it('can create address from public key for custom network', () => {
            ...
            const publicKey = format_1.Convert.hexToUint8('3485D98EFD7EB07ADAFCFD1A157D89DE2796A95E780813C0258AF3F5F84ED8CB');
            // Act:
            const decoded = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Public_Test_Identifier);
            // Assert:
            chai_1.expect(decoded[0]).to.equal(Network_Public_Test_Identifier);
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded)).to.equal(true);
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal(expectedHex);
        });
        it('address calculation is deterministic', () => {
            ...
            const publicKey = format_1.Convert.hexToUint8('3485D98EFD7EB07ADAFCFD1A157D89DE2796A95E780813C0258AF3F5F84ED8CB');
            // Act:
            const decoded1 = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Mijin_Identifier);
            const decoded2 = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Mijin_Identifier);
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded1)).to.equal(true);
            chai_1.expect(decoded1).to.deep.equal(decoded2);
        });
        it('different public keys result in different addresses', () => {
            // Arrange:
            const publicKey1 = format_1.Convert.hexToUint8('1464953393CE96A08ABA6184601FD08864E910696B060FF7064474726E666CA8');
            const publicKey2 = format_1.Convert.hexToUint8('b4f12e7c9f6946091e2cb8b6d3a12b50d17ccbbf646386ea27ce2946a7423dcf');
            // Act:
            const decoded1 = format_1.RawAddress.publicKeyToAddress(publicKey1, Network_Mijin_Identifier);
            const decoded2 = format_1.RawAddress.publicKeyToAddress(publicKey2, Network_Mijin_Identifier);
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded1)).to.equal(true);
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded2)).to.equal(true);
            chai_1.expect(decoded1).to.not.deep.equal(decoded2);
        });
        it('different networks result in different addresses', () => {
            // Arrange:
            const publicKey = format_1.Convert.hexToUint8('b4f12e7c9f6946091e2cb8b6d3a12b50d17ccbbf646386ea27ce2946a7423dcf');
            // Act:
            const decoded1 = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Mijin_Identifier);
            const decoded2 = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Public_Test_Identifier);
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded1)).to.equal(true);
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded2)).to.equal(true);
            chai_1.expect(decoded1).to.not.deep.equal(decoded2);
        ...
        it('can create address from public key using NIS1 schema', () => {
            ...
            const publicKey = format_1.Convert.hexToUint8('3485D98EFD7EB07ADAFCFD1A157D89DE2796A95E780813C0258AF3F5F84ED8CB');
            // Act:
            const decoded = format_1.RawAddress.publicKeyToAddress(publicKey, Network_Public_Test_Identifier, 1);
            // Assert:
            chai_1.expect(decoded[0]).to.equal(Network_Public_Test_Identifier);
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded, crypto_1.SignSchema.KECCAK_REVERSED_KEY)).to.equal(true);
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal(keccakHex);
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).not.to.equal(nonKeccakHex);
        });
    });
    describe('isValidAddress', () => {
        it('returns true for valid address', () => {
            ...
            const decoded = format_1.Convert.hexToUint8(validHex);
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded)).to.equal(true);
        });
        it('returns false for address with invalid checksum', () => {
            ...
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidAddress(decoded)).to.equal(false);
        });
    ...
    describe('isValidEncodedAddress', () => {
        it('returns true for valid encoded address', () => {
            ...
            const encoded = 'NAR3W7B4BCOZSZMFIZRYB3N5YGOUSWIYJCJ6HDFG';
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidEncodedAddress(encoded)).to.equal(true);
        });
        it('returns false for invalid encoded address', () => {
            ...
            // Assert:
            chai_1.expect(format_1.RawAddress.isValidEncodedAddress(encoded)).to.equal(false);
        });
    ...
    describe('NIS1 test vector [PublicNet] - PublicKey to Address', () => {
        it('can create Address from NIS public Key', () => {
            ...
            // Sanity:
            chai_1.expect(Public_Keys.length).equal(Addresses.length);
            for (let i = 0; i < Public_Keys.length; ++i) {
                // Arrange:
                const publicKeyHex = Public_Keys[i];
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.MAIN_NET, crypto_1.SignSchema.KECCAK_REVERSED_KEY));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
        });
    });
    /**
    ...
    describe('NIS1 test vector [PublicTest] - PublicKey to Address', () => {
        it('can create Address from NIS public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.TEST_NET, crypto_1.SignSchema.KECCAK_REVERSED_KEY));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
    ...
    describe('NIS1 test vector [MIJIN] - PublicKey to Address', () => {
        it('can create Address from NIS public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.MIJIN, crypto_1.SignSchema.KECCAK_REVERSED_KEY));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
    ...
    describe('Catapult test vector [PublicNet] - PublicKey to Address', () => {
        it('can create Address from Catapult public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.MAIN_NET));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
    ...
    describe('Catapult test vector [PublicTest] - PublicKey to Address', () => {
        it('can create Address from Catapult public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.TEST_NET));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
    ...
    describe('Catapult test vector [MIJIN] - PublicKey to Address', () => {
        it('can create Address from Catapult public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.MIJIN));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
    ...
    describe('Catapult test vector [MIJIN_TEST] - PublicKey to Address', () => {
        it('can create Address from Catapult public Key', () => {
            ...
            for (let i = 0; i < Public_Keys.length; ++i) {
                ...
                const expectedAddress = Addresses[i];
                // Act:
                const result = format_1.RawAddress.addressToString(format_1.RawAddress.publicKeyToAddress(format_1.Convert.hexToUint8(publicKeyHex), model_1.NetworkType.MIJIN_TEST));
                // Assert:
                const message = ` from ${publicKeyHex}`;
                chai_1.expect(result.toUpperCase(), `public ${message}`).equal(expectedAddress.toUpperCase());
            }
...
//# sourceMappingURL=RawAddress.spec.js.map