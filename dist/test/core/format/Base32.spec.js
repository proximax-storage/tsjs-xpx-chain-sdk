...
const Base32_1 = require("../../../src/core/format/Base32");
describe('base32', () => {
    const Test_Vectors = [{
    ...
    describe('encode', () => {
        it('can convert empty input', () => {
            // Act:
            const encoded = Base32_1.Base32.Base32Encode(new Uint8Array([]));
            // Assert:
            chai_1.expect(encoded).to.equal('');
        });
        it('can convert test vectors', () => {
            // Arrange:
            for (const sample of Test_Vectors) {
                const input = format_1.Convert.hexToUint8(sample.decoded);
                // Act:
                const encoded = Base32_1.Base32.Base32Encode(input);
                // Assert:
                chai_1.expect(encoded, `input ${sample.decoded}`).to.equal(sample.encoded);
            }
        });
        it('accepts all byte values', () => {
            ...
            // Act:
            const encoded = Base32_1.Base32.Base32Encode(data);
            // Assert:
        ...
        it('throws if input size is not a multiple of block size', () => {
            ...
            for (let i = 2; 10 > i; i += 2) {
                ...
                // Act + Assert:
                chai_1.expect(() => {
                    Base32_1.Base32.Base32Encode(input);
    ...
    describe('decode', () => {
        it('can convert empty input', () => {
            // Act:
            const decoded = Base32_1.Base32.Base32Decode('');
            // Assert:
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal('');
        });
        it('can convert test vectors', () => {
            // Arrange:
            for (const sample of Test_Vectors) {
                // Act:
                const decoded = Base32_1.Base32.Base32Decode(sample.encoded);
                // Assert:
                chai_1.expect(format_1.Convert.uint8ToHex(decoded), `input ${sample.encoded}`).to.equal(sample.decoded);
            }
        });
        it('accepts all valid characters', () => {
            // Act:
            const decoded = Base32_1.Base32.Base32Decode('ABCDEFGHIJKLMNOPQRSTUVWXYZ234567');
            // Assert:
            chai_1.expect(format_1.Convert.uint8ToHex(decoded)).to.equal('00443214C74254B635CF84653A56D7C675BE77DF');
        });
        it('throws if input size is not a multiple of block size', () => {
            // Arrange:
            for (let i = 1; 8 > i; ++i) {
                ...
                // Act + Assert:
                chai_1.expect(() => {
                    Base32_1.Base32.Base32Decode(input);
        ...
        it('throws if input contains an invalid character', () => {
            // Arrange:
            const illegalInputs = [
                'NC5J5DI2URIC4H3T3IMXQS21PWQWZIPEV6EV7LAS',
                'NBGCMBPFWNTLXFF4GB2V5SPV!V2OQD6JFA6SBYUD',
    ...
    describe('roundtrip', () => {
        it('decode -> encode', () => {
            ...
            for (const input of inputs) {
                // Act:
                const decoded = Base32_1.Base32.Base32Decode(input);
                const result = Base32_1.Base32.Base32Encode(decoded);
                // Assert:
                chai_1.expect(result, `input ${input}`).to.equal(input);
            }
        });
        it('encode -> decode', () => {
            // Arrange: inputs
            ...
            for (const input of inputs) {
                // Act:
                const encoded = Base32_1.Base32.Base32Encode(format_1.Convert.hexToUint8(input));
                const result = Base32_1.Base32.Base32Decode(encoded);
                // Assert:
                chai_1.expect(format_1.Convert.uint8ToHex(result), `input ${input}`).to.equal(input);
            }
        });
    });
});
//# sourceMappingURL=Base32.spec.js.map