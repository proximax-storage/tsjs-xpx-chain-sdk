...
describe('id generator', () => {
    ...
    function addBasicTests(generator) {
        it('produces different results for different names', () => {
            // Assert:
            ['bloodyrookie.alice', 'nem.mex', 'bloodyrookie.xem', 'bloody_rookie.xem'].forEach((name) => chai_1.expect(generator(name), `nem.xem vs ${name}`).to.not.equal(generator('nem.xem')));
        });
        it('rejects names with uppercase characters', () => {
            // Assert:
            ['NEM.xem', 'NEM.XEM', 'nem.XEM', 'nEm.XeM', 'NeM.xEm'].forEach((name) => chai_1.expect(() => generator(name), `name ${name}`).to.throw('invalid part name'));
        });
        it('rejects improper qualified names', () => {
            // Assert:
            ['.', '..', '...', '.a', 'b.', 'a..b', '.a.b', 'b.a.'].forEach((name) => chai_1.expect(() => generator(name), `name ${name}`).to.throw('empty part'));
        });
        it('rejects improper part names', () => {
            // Assert:
            ['alpha.bet@.zeta', 'a!pha.beta.zeta', 'alpha.beta.ze^a'].forEach((name) => chai_1.expect(() => generator(name), `name ${name}`).to.throw('invalid part name'));
        });
        it('rejects empty string', () => {
            // Assert:
            chai_1.expect(() => generator(''), 'empty string').to.throw('having zero length');
        });
    }
    describe('generate mosaic id', () => {
        it('generates correct well known id', () => {
            // Assert:
            chai_1.expect(format_1.IdGenerator.generateMosaicId(basicMosaicInfo.nonce, basicMosaicInfo.publicId))
        ...
        it('generates correct mosaicId given nonce and public key', () => {
            mosaicTestVector.rows.map((row, i) => {
                const pubKey = format_1.Convert.hexToUint8(row.publicKey);
                const nonce = format_1.Convert.hexToUint8(row.nonce).reverse(); // Little-Endianness!
                const mosaicId = format_1.IdGenerator.generateMosaicId(nonce, pubKey);
                const expectedId = format_1.RawUInt64.fromHex(row.expectedMosaicId);
                // Assert:
                chai_1.expect(mosaicId)
    ...
    describe('generate namespace paths', () => {
        it('generates correct well known root path', () => {
            // Act:
            const path = format_1.IdGenerator.generateNamespacePath('nem');
            // Assert:
            chai_1.expect(path.length).to.equal(1);
            chai_1.expect(path[0]).to.deep.equal(constants.nem_id);
        });
        it('generates correct well known child path', () => {
            // Act:
            const path = format_1.IdGenerator.generateNamespacePath('nem.xem');
            // Assert:
            chai_1.expect(path.length).to.equal(2);
            chai_1.expect(path[0]).to.deep.equal(constants.nem_id);
            chai_1.expect(path[1]).to.deep.equal(constants.xem_id);
        });
        it('supports multi level namespaces', () => {
            ...
            // Assert:
            chai_1.expect(format_1.IdGenerator.generateNamespacePath('foo.bar.baz')).to.deep.equal(expected);
        });
        it('rejects names with too many parts', () => {
            // Assert:
            ['a.b.c.d', 'a.b.c.d.e'].forEach((name) => chai_1.expect(() => format_1.IdGenerator.generateNamespacePath(name), `name ${name}`).to.throw('too many parts'));
        });
        it('rejects improper qualified names', () => {
            // Assert:
            ['a:b:c', 'a::b'].forEach((name) => chai_1.expect(() => format_1.IdGenerator.generateNamespacePath(name), `name ${name}`).to.throw('invalid part name'));
        });
        addBasicTests(format_1.IdGenerator.generateNamespacePath);
    });
});
//# sourceMappingURL=IdGenerator.spec.js.map