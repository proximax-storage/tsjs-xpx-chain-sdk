...
class Listener {
     ...
     */
    multisigAccountAdded(address) {
        this.subscribeTo(`modifyMultisigAccount/${address.plain()}`);
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.modifyMultisigAccount), operators_1.filter((_) => _.message instanceof ModifyMultisigAccountTransaction_1.ModifyMultisigAccountTransaction), operators_1.map((_) => _.message), operators_1.filter((_) => this.accountAddedToMultiSig(_, address)));
    }
    /**
