...
export class RawAddress {
    ...
    public static addressToString = (decoded: Uint8Array): string => {
        if (RawAddress.constants.sizes.addressDecoded !== decoded.length) {
            throw Error(`${Convert.uint8ArrayToHex(decoded)} does not represent a valid decoded address`);
        }
    ...
    public static isValidAddress = (decoded: Uint8Array): boolean => {
        ...
        RawArray.copy(checksum, RawArray.uint8View(hash.digest()), RawAddress.constants.sizes.checksum);
