...
import { Builder } from '../../infrastructure/builders/VrfLinkTransaction';
...
/**
 * Announce an VrfLinkTransaction, must use v2 account
 * By doing so, you can enable delegated harvesting
 */
export class VrfLinkTransaction extends Transaction {
    /**
     * Create a Vrf link transaction object
     * @param deadline - The deadline to include the transaction.
     * @param remoteAccountKey - The public key of the v2 account.
     * @param linkAction - Link or unlink
     * @param maxFee - (Optional) Max fee defined by the sender
     * @returns {VrfLinkTransaction}
     */
    public static create(deadline: Deadline,
                         remoteAccountKey: string,
                         linkAction: AccountLinkAction,
                         networkType: NetworkType,
                         maxFee?: UInt64): VrfLinkTransaction {
        return new VrfLinkTransactionBuilder()
            .networkType(networkType)
            .deadline(deadline)
            .maxFee(maxFee)
    ...
    constructor(networkType: NetworkType,
                ...
                maxFee: UInt64,
                /**
                 * The public key of the Vrf account
                 */
     ...
     * @description get the byte size of a VrfLinkTransaction
     * @returns {number}
     * @memberof VrfLinkTransaction
     */
    public get size(): number {
        return VrfLinkTransaction.calculateSize();
    }
...
export class VrfLinkTransactionBuilder extends TransactionBuilder {
    ...
    public build(): VrfLinkTransaction {
        return new VrfLinkTransaction(
            this._networkType,
            this._version || TransactionTypeVersion.Vrf_Key_Link,
            this._deadline ? this._deadline : this._createNewDeadlineFn(),
            this._maxFee ? this._maxFee : calculateFee(VrfLinkTransaction.calculateSize(), this._feeCalculationStrategy),
