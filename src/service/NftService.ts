...
export class NftService {
    ...
    async findNft(nftCandidates: MosaicId[]){
        ...
        let nftAssets = assetsInfo.filter(x => (!x.isSupplyMutable() || x.isSupplyForceImmutable()) && x.divisibility === 0)
                                    .map(x=> { return { 
                                        assetId: x.mosaicId,
    ...
    async createMultisigNFTPayload(myAcc: Account, multisigAcc: PublicAccount | string, 
        ...
        try {
            const chainConfigHttp = new NetworkConfigHttp(this.bcApiUrl);
            const chainHttp = new ChainHttp(this.bcApiUrl);
            const chainHeight = await firstValueFrom(chainHttp.getBlockchainHeight());
            const chainConfig = await firstValueFrom(chainConfigHttp.getNetworkConfig(Number(chainHeight.toBigInt())));
        ...
        if(!cosignerCheckResult.aggComplete){
            ...
            try {
                const nodeHttp = new NodeHttp(this.bcApiUrl, this.networkHttp);
